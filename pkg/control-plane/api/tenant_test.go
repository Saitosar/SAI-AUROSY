...
func setupTenantTestServer(t *testing.T) (*Server, *registry.MemoryStore) {
	...
	srv := NewServer(reg, bus, apiKeyStore, taskStore, scenarioCatalog, nil, wfCatalog, wfRunStore, wfRunner, nil, nil, nil, analyticsStore, edgeStore, nil, nil, nil, nil, nil, nil, nil)
	return srv, reg
}
...
func TestTenantEnforcement_CancelTask(t *testing.T) {
	...
	wfRunStore := orchestration.NewMemoryRunStore()
	wfRunner := orchestration.NewRunner(wfCatalog, wfRunStore, taskStore, scenarioCatalog, reg)
	srv2 := NewServer(reg, bus, newMockAPIKeyStore(), taskStore, scenarioCatalog, nil, wfCatalog, wfRunStore, wfRunner, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil)
...
func TestTenantEnforcement_ListEdges(t *testing.T) {
	...
	wfRunStore := orchestration.NewMemoryRunStore()
	wfRunner := orchestration.NewRunner(wfCatalog, wfRunStore, taskStore, scenarioCatalog, reg)
	srv2 := NewServer(reg, bus, newMockAPIKeyStore(), taskStore, scenarioCatalog, nil, wfCatalog, wfRunStore, wfRunner, nil, nil, nil, &mockAnalyticsStore{}, edgeStore, nil, nil, nil, nil, nil, nil, nil)
