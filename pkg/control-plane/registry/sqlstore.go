...
func (s *SQLStore) Add(r *hal.Robot) {
	...
	if s.driver == "pgx" {
		s.db.Exec(s.ph("INSERT INTO robots (id, vendor, model, adapter_endpoint, tenant_id, capabilities, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT (id) DO UPDATE SET vendor=?, model=?, adapter_endpoint=?, tenant_id=?, capabilities=?, updated_at=?"),
			r.ID, r.Vendor, r.Model, r.AdapterEndpoint, r.TenantID, string(capsJSON), r.CreatedAt, r.UpdatedAt,
			r.Vendor, r.Model, r.AdapterEndpoint, r.TenantID, string(capsJSON), r.UpdatedAt,
		)
	} else {
		s.db.Exec("INSERT OR REPLACE INTO robots (id, vendor, model, adapter_endpoint, tenant_id, capabilities, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
			r.ID, r.Vendor, r.Model, r.AdapterEndpoint, r.TenantID, string(capsJSON), r.CreatedAt, r.UpdatedAt,
		)
	}
}
...
func (s *SQLStore) Get(id string) *hal.Robot {
	...
	err := s.db.QueryRow(
		s.ph("SELECT id, vendor, model, adapter_endpoint, tenant_id, capabilities, created_at, updated_at FROM robots WHERE id=?"),
		id,
	).Scan(&r.ID, &r.Vendor, &r.Model, &r.AdapterEndpoint, &r.TenantID, &capsJSON, &r.CreatedAt, &r.UpdatedAt)
...
func (s *SQLStore) List() []hal.Robot {
	rows, err := s.db.Query("SELECT id, vendor, model, adapter_endpoint, tenant_id, capabilities, created_at, updated_at FROM robots ORDER BY id")
	if err != nil {
		return nil
	}
	defer rows.Close()
	...
	for rows.Next() {
		var r hal.Robot
		var capsJSON sql.NullString
		if err := rows.Scan(&r.ID, &r.Vendor, &r.Model, &r.AdapterEndpoint, &r.TenantID, &capsJSON, &r.CreatedAt, &r.UpdatedAt); err != nil {
			continue
		}
		if capsJSON.Valid && capsJSON.String != "" {
			_ = json.Unmarshal([]byte(capsJSON.String), &r.Capabilities)
